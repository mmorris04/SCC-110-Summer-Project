...
public class Player {
	...
	*/
    public void deflectPuck(Ball puckBall, SoundPlayer soundPlayer) {
        ...
        if (Double.isNaN(xSpeed2)) {
            xSpeed2 = (initialXSpeed2*-1)/4; 
        }
        if (Double.isNaN(ySpeed2)) {
            ySpeed2 = (initialYSpeed2*-1)/4; 
        }
	...
	*/
    public boolean isTouchingPuck(Ball puck) {
        ...
        double a = puck.getXPosition()-Mallet.getXPosition();
        double b = puck.getYPosition()-Mallet.getYPosition();
        double c2 = (a * a) + (b * b);
        double c = Math.sqrt(c2);

        // If distance between puck and player is less than or equal to their radius' combined then they are touching
        return (c <= ((Mallet.getSize()/2)+(puck.getSize()/2)));
    }
	...
	*/
    public void movePlayer(double newXSpeed, double newYSpeed, Pitch pitch, SoundPlayer soundPlayer) {
